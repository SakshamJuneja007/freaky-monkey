...
def _local_now(observed_at: float | None = None) -> datetime:
    # Semantic date resolution must never depend on the machine timezone.
    # DEIMOS uses its canonical event timezone unless a more specific event
    # timezone is supplied by the event layer.
...
def _source_timezone_label(message: WhatsAppMessage) -> str:
    ...
    # BrowserSkill/connector payloads may preserve the original ISO timestamp
    # without separately naming its timezone. Derive the offset here instead of
    # reporting UNKNOWN. An explicit +00:00 offset is unambiguously UTC.
...
_EVENT_TYPE_PATTERNS: tuple[tuple[IntelligenceEventType, re.Pattern[str]], ...] = (
    (IntelligenceEventType.MEETING, re.compile(
        r"\b(?:meeting|meet|let(?:\'s| us)\s+meet|we\s+(?:will|\'ll)\s+meet)\b", re.I
    )),
)


def _semantic_event_type(text: str) -> IntelligenceEventType | None:
    """Determine event semantics before looking for an event clock.

    This is deliberately small and uses the existing intelligence taxonomy. A
    clock token is never considered evidence for this decision.
    """
    lower = _normalize_text(text).casefold()
    if not lower:
        return None

    # Explicit event references used by lifecycle updates. These are evidence
    # of an existing event, but only when an event noun/reference is present.
    if re.search(r"\b(?:the|that|same)\s+(?:meeting|appointment|class|session|event)\b", lower):
        return IntelligenceEventType.MEETING
    if re.search(r"\b(?:move|reschedule|rescheduled|shift|shifted|changed|change)\b.*\b(?:meeting|appointment|class|session|event)\b", lower):
        return IntelligenceEventType.MEETING
    if re.search(r"\b(?:cancel|cancelled|canceled|confirm|confirmed)\b.*\b(?:meeting|appointment|class|session|event)\b", lower):
        return IntelligenceEventType.MEETING

    for kind, pattern in _EVENT_TYPE_PATTERNS:
        if pattern.search(lower):
            return kind

    # Other supported scheduled-event language is intentionally left to the
    # existing non-meeting intelligence taxonomy. This gate must not collapse
    # every event category into MEETING.
    return None


def _event_context_detected(text: str) -> bool:
    return _semantic_event_type(text) is not None
...
def _extract_semantic_messages(raw: Any, *, conversation_hint: str = "", observed_at: float | None = None) -> list[WhatsAppMessage]:
    ...
    if elements:
        ...
        # BrowserSkill test/adapter payloads may expose ``elements`` as plain
        # dictionaries rather than SemanticElement objects. Treat those fields
        # as structured metadata and keep their text/name separate from the
        # message clock.
        if isinstance(raw, dict) and isinstance(raw.get("elements"), (list, tuple)):
            for element in raw["elements"]:
                if not isinstance(element, dict):
                    continue
                role = str(element.get("role") or element.get("type") or element.get("element_type") or "").casefold()
                name = _normalize_text(element.get("name") or element.get("label") or "")
                value = _normalize_text(element.get("value") or element.get("text") or element.get("content") or "")
                if role in _SEMANTIC_ROLES and (name or value):
                    add_semantic(str(element.get("ref") or element.get("id") or ""), role, _normalize_text(f"{name} {value}"), element)
    ...
    for index, (ref, role, value) in enumerate(semantic):
        ...
        # When BrowserSkill gives a single semantic label containing both the
        # visible message clock and message body, keep the clock as timestamp
        # provenance and keep only the natural-language body for semantics.
        # This is structure-aware: it does not assume the first/last clock is
        # an event time.
        leading_clock = _TIME_TOKEN_RE.match(combined)
        semantic_kind = _semantic_event_type(combined)
        event_clock = _event_time_match(combined, meeting_context=semantic_kind is IntelligenceEventType.MEETING) if semantic_kind else None
        time_token_for_body: str | None = event_clock.group(0) if event_clock else None
        if leading_clock is not None and event_clock is not None and leading_clock.span() != event_clock.span():
            parsed_leading = _parse_clock_match(leading_clock)
            if parsed_leading is not None:
                hour, minute = parsed_leading
                base_clock = date_context or _local_now(observed_at)
                source_dt = base_clock.replace(hour=hour, minute=minute, second=0, microsecond=0)
                timestamp = source_dt.timestamp()
                time_token = time_token_for_body
            else:
        ...
        if timestamp is None:
            continue
        # Event clocks are message content, not metadata. Only strip the
        # timestamp token when there is no semantic event clock.
        body_time_token = None if (semantic_kind is not None and event_clock is not None) else time_token
        body, parsed_sender = _semantic_message_text(combined, body_time_token, conversation_hint)
        if leading_clock is not None and event_clock is not None and leading_clock.span() != event_clock.span():
            leading_text = leading_clock.group(0)
            body = _normalize_text(re.sub(r"^" + re.escape(leading_text) + r"\s*", "", body, count=1, flags=re.IGNORECASE))
    ...
    def repair_meeting_event_times(self, *, on_debug: Callable[[str], None] | None = None) -> int:
        ...
        for row in rows:
            ...
            for evidence in evidence_rows:
                ...
                if not claim:
                    continue
                # Legacy repair is allowed only when the persisted evidence
                # itself establishes semantic event context. A bare UI/message
                # clock is never repair evidence.
                if not _event_context_detected(claim):
                    continue
...
class WhatsAppIntelligence:
    ...
    def _event_proposal(self, message: WhatsAppMessage) -> EventProposal:
        ...
        # Gate the entire lifecycle on semantic event context. In particular,
        # time_match alone can never create, update, cancel, confirm, merge,
        # supersede, or repair an event.
        event_type = _semantic_event_type(text)
        date_match = re.search(
            r"\b(today|tomorrow|tonight|monday|tuesday|wednesday|thursday|friday|saturday|sunday|next week)\b",
            lower,
        )
        cancellation = bool(re.search(r"\b(?:cancelled|canceled|cancel|cancel hai)\b", lower))
        confirmation = bool(re.search(r"\b(?:confirmed|confirm)\b", lower))
        correction = bool(re.search(r"\b(?:actually|move(?: it)?|moved|reschedule(?:d)?|change(?:d)?|shift(?:ed)?|kar di|kar diya)\b", lower))
        ...
        # A bare clock, date token, confirmation word, platform name, or URL is
        # not by itself enough to enter event mutation. Updates require an
        # explicit event reference/correction.
        if event_type is None:
            return EventProposal("NO_EVENT", None, evidence_message_id=message.message_id)

        meeting_context = event_type is IntelligenceEventType.MEETING
        time_match = _event_time_match(text, meeting_context=meeting_context)
        ...
        # A genuine event without a clock is still a valid event (for example a
        # later cancellation/confirmation), but a time-containing message only
        # reaches this point after semantic context was established.
        explicit_reference = bool(re.search(r"\b(?:that meeting|same meeting|as discussed|same link)\b", lower))
        has_event_noun = event_type is not None
        if cancellation:
            action = "CANCEL_EVENT"
        elif confirmation and has_event_noun:
            action = "CONFIRM_EVENT"
        elif correction or explicit_reference:
            action = "UPDATE_EVENT"
        else:
            action = "CREATE_EVENT"

        title = {
            IntelligenceEventType.MEETING: "Meeting",
            IntelligenceEventType.DEADLINE: "Deadline",
            IntelligenceEventType.ASSIGNMENT: "Assignment",
            IntelligenceEventType.TASK: "Task",
        }.get(event_type, event_type.value.title())

        return EventProposal(
            action=action, event_type=event_type, title=title,
            date_expression=date_match.group(1) if date_match else None,
            resolved_date=resolved_date,
            time_expression=time_match.group(0) if time_match else None,
            resolved_time=resolved_time, timezone=_timezone_label(event_tz),
            platform=platform_match.group(1).title() if platform_match else None,
            meeting_link=link_match.group(0) if link_match else None,
            confidence=0.90 if cancellation or confirmation else 0.86,
            referenced_event_hint=("explicit" if explicit_reference else ("implicit" if correction else None)),
            cancellation=cancellation, confirmation=confirmation, correction=correction,
            evidence_message_id=message.message_id, extracted_claim=text,
        )
        ...
        if proposal.action in {"UPDATE_EVENT", "CONFIRM_EVENT", "CANCEL_EVENT"}:
            ...
            if scored:
                if len(scored) == 1 and scored[0][0] >= 0.45:
                    ...
                    existing = scored[0][1]
            if existing is None:
                # A semantically explicit confirmation can establish the event
                # when no prior event exists. Updates/cancellations still fail
                # closed rather than inventing a target event.
                if proposal.action != "CONFIRM_EVENT":
                    if scored:
                        self.on_debug(f"WHATSAPP_EVENT: AMBIGUOUS candidates={len(scored)} action=NEEDS_CLARIFICATION")
                    return None
