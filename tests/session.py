...
from .whatsapp_control import WhatsAppControlLease, WhatsAppOwner
from .whatsapp_intelligence import WhatsAppIntelligence, WhatsAppIntelligenceStore
...
class Session:
    ...
    #: WhatsApp has one explicit reusable browser resource whose automation
    #: ownership can be handed to the human without tearing down the session.
    _whatsapp_control: WhatsAppControlLease = field(
        default_factory=WhatsAppControlLease, repr=False, compare=False,
    )
    _whatsapp_intelligence: WhatsAppIntelligence | None = field(default=None, repr=False, compare=False)
    _whatsapp_intelligence_browser: BrowserSkillAdapter | None = field(default=None, repr=False, compare=False)
    ...
    def __post_init__(self) -> None:
        ...
        # P3.3: restore an already-enabled WhatsApp intelligence observer for
        # this live Session.  The intelligence store is persistent, while the
        # observer itself is intentionally session-scoped.  Do not wait for a
        # future control command to instantiate it: otherwise a persisted
        # ``enabled`` state becomes inert after a DEIMOS restart.
        self._resume_whatsapp_intelligence_if_enabled()
    ...
    def _browser_for_task(
        self,
        task_id: str,
        *,
        resource_key: str | None = None,
        allow_human_control_transfer: bool = False,
    ) -> BrowserSkillAdapter:
        ...
        # WhatsApp is the first explicit shared browser resource: its authenticated
        # BrowserSkill session survives a Human handoff and is reacquired later.
        # Other browser resources retain their existing task-scoped isolation.
        if resource_key == "whatsapp":
            if self._whatsapp_control.human_owned and not allow_human_control_transfer:
                raise RuntimeError("whatsapp_resource_human_owned")
            if self._whatsapp_control.browser is not None:
                self._browser_tasks[key] = self._whatsapp_control.browser
                return self._whatsapp_control.browser
        ...
        try:
            ...
            pass
        if resource_key == "whatsapp":
            backend = self._whatsapp_control.bind_browser(backend)
    ...
    def _whatsapp_intelligence_service(self) -> WhatsAppIntelligence:
        if self._whatsapp_intelligence is None:
            path = os.getenv("DEIMOS_WHATSAPP_INTELLIGENCE", "").strip()
            store = WhatsAppIntelligenceStore(path or (Path(self.workspace or Path.cwd()) / ".agent_memory" / "whatsapp_intelligence.sqlite3"))
            self._whatsapp_intelligence = WhatsAppIntelligence(
                store,
                can_observe=lambda: self._whatsapp_control.deimos_owned and self._whatsapp_control.active_task_id is None and not self._closing,
                on_debug=(lambda line: self.narrator.note(line)) if self.debug else (lambda _line: None),
                on_intelligence=self._present_whatsapp_intelligence,
            )
        return self._whatsapp_intelligence

    def _whatsapp_intelligence_browser_for_observation(self) -> BrowserSkillAdapter:
        browser = self._whatsapp_control.browser
        if browser is None:
            browser = self._browser_for_task(
                "whatsapp-intelligence", resource_key="whatsapp",
            )
        self._whatsapp_intelligence_browser = browser
        return browser

    def _resume_whatsapp_intelligence_if_enabled(self) -> bool:
        """Start the session-scoped observer when persisted intelligence is enabled.

        This is deliberately a small lifecycle adapter around the existing
        ``WhatsAppIntelligence.start_observing`` API.  The engine owns polling,
        cutoff enforcement, deduplication and event extraction; Session only
        decides whether the existing WhatsApp resource may be observed.
        """
        if self._closing or not self._whatsapp_control.deimos_owned:
            return False
        service = self._whatsapp_intelligence_service()
        if not service.store.state().enabled:
            return False
        if self._whatsapp_control.active_task_id is not None:
            return False
        try:
            browser = self._whatsapp_intelligence_browser_for_observation()
            service.start_observing(browser)
            return True
        except Exception as exc:
            # Intelligence is advisory/read-only. A browser startup problem
            # must never terminate the conversational Session.
            service.on_debug(f"WHATSAPP_INTELLIGENCE: OBSERVER_START_FAILED error={type(exc).__name__}")
            return False

    def _enable_whatsapp_intelligence(self) -> Turn:
        task = UserTask(raw="enable WhatsApp intelligence", text="enable WhatsApp intelligence", source="text", status="accepted")
        if self._whatsapp_control.human_owned:
            return self._refused(task, "WhatsApp Web is under human control. Take over WhatsApp before enabling intelligence.")
        service = self._whatsapp_intelligence_service()
        state = service.enable()
        browser = self._whatsapp_intelligence_browser_for_observation()
        service.start_observing(browser)
        if self.debug:
            self._debug_route("WHATSAPP_INTELLIGENCE", control="ENABLE")
        return self._refused(task, "WHATSAPP_INTELLIGENCE: ENABLED. Message intelligence uses the previous 5 days; persistent event memory is retained beyond that window.")

    def _authorize_whatsapp_intelligence(self, target: str) -> Turn:
        task = UserTask(raw=f"allow WhatsApp intelligence to observe {target}", text=target, source="text", status="accepted")
        if self._whatsapp_control.human_owned:
            return self._refused(task, "WhatsApp Web is under human control. Take over WhatsApp before granting intelligence access.")
        service = self._whatsapp_intelligence_service()
        try:
            normalized = service.store.authorize_target(target)
        except ValueError:
            return self._refused(task, "WHATSAPP_INTELLIGENCE: AUTHORIZATION_REJECTED. Target name is empty.")
        self._resume_whatsapp_intelligence_if_enabled()
        return self._refused(task, f"WHATSAPP_INTELLIGENCE: AUTHORIZED target={normalized!r}. Only this WhatsApp contact/group may be opened and observed.")

    def _revoke_whatsapp_intelligence(self, target: str) -> Turn:
        task = UserTask(raw=f"revoke WhatsApp intelligence from {target}", text=target, source="text", status="accepted")
        service = self._whatsapp_intelligence_service()
        removed = service.store.revoke_target(target)
        if not removed:
            return self._refused(task, f"WHATSAPP_INTELLIGENCE: NOT_AUTHORIZED target={target!r}.")
        return self._refused(task, f"WHATSAPP_INTELLIGENCE: REVOKED target={target!r}.")
    ...
    def _authorize_whatsapp_intelligence(self, target: str) -> Turn:
        ...
        try:
            ...
            return self._refused(task, "WHATSAPP_INTELLIGENCE: AUTHORIZATION_REJECTED. Target name is empty.")
        self._resume_whatsapp_intelligence_if_enabled()
        return self._refused(task, f"WHATSAPP_INTELLIGENCE: AUTHORIZED target={normalized!r}.")

    def _revoke_whatsapp_intelligence(self, target: str) -> Turn:
        task = UserTask(raw=f"revoke WhatsApp intelligence from {target}", text=target, source="text", status="accepted")
        service = self._whatsapp_intelligence_service()
        if not service.store.revoke_target(target):
            return self._refused(task, f"WHATSAPP_INTELLIGENCE: NOT_AUTHORIZED target={target!r}.")
        return self._refused(task, f"WHATSAPP_INTELLIGENCE: REVOKED target={target!r}.")

    def _disable_whatsapp_intelligence(self) -> Turn:
        task = UserTask(raw="disable WhatsApp intelligence", text="disable WhatsApp intelligence", source="text", status="accepted")
        service = self._whatsapp_intelligence_service()
        service.disable()
        service.stop_observing()
        if self.debug:
            self._debug_route("WHATSAPP_INTELLIGENCE", control="DISABLE")
        return self._refused(task, "WHATSAPP_INTELLIGENCE: DISABLED.")

    def close(self, timeout_s: float = 15.0) -> None:
        """Close interface-owned resources and drain the background lane."""
        self._closing = True
        if self._whatsapp_intelligence is not None:
            self._whatsapp_intelligence.stop_observing()
        ...
        try:
            ...
            try:
                ...
                pass
            backends = list({id(backend): backend for backend in self._browser_tasks.values()}.values())
        ...
        if task.status == "unregistered":
            ...
            if fast.route is not None:
                ...
                if fast.route.action_kind == "whatsapp_send_message":
                    if self._whatsapp_control.human_owned:
                        return self._refused(
                            replace(task, task_id=fast_task_id, status="accepted"),
                            "WhatsApp Web is currently under your control. Ask me to take over WhatsApp before requesting automation.",
                        )
    ...
    def _browser_resource_key(prepared: Prepared) -> str | None:
        ...
        if prepared.browser_resource_key:
            ...
            # one.
            if prepared.browser_resource_key == "whatsapp":
                return "whatsapp"
            if prepared.browser_resource_key in {"browser", "youtube", "gmail"}:
                return "browser"
            return prepared.browser_resource_key
        text = f"{prepared.task.text} {prepared.goal}".casefold()
        if "whatsapp" in text:
            return "whatsapp"
        if "youtube" in text or "gmail" in text or "mail.google.com" in text or re.search(r"\bplay\s+.+", text):
            return "browser"
        return None
    ...
    def _run(self, prepared: Prepared) -> Turn:
        ...
        # operation for this task. WhatsApp additionally has an explicit Human
        # ownership lease, so a human-owned resource is rejected before any
        # BrowserSkill operation can occur.
        resource_key = self._browser_resource_key(prepared)
        if resource_key == "whatsapp" and self._whatsapp_control.human_owned:
            result = AgentResult(
                request=task.text,
                task_id=task.task_id,
                status=TaskStatus.POLICY_BLOCKED,
                detail="WhatsApp Web is owned by the human; DEIMOS automation is paused.",
            )
            self._runtime_transition(
                runtime_task_id,
                "FAILED",
                event_type="WHATSAPP_AUTOMATION_BLOCKED",
                failure_state="whatsapp_human_owned",
                failure_category="RESOURCE_OWNERSHIP",
            )
            return Turn(task=task, reply="WhatsApp Web is currently under your control. Ask me to take over WhatsApp before requesting automation.", result=result)
        ...
        whatsapp_active = resource_key == "whatsapp"
        if whatsapp_active:
            self._whatsapp_control.active_task_id = runtime_task_id
        ...
        try:
            ...
            if whatsapp_active and self._whatsapp_control.active_task_id == runtime_task_id:
                self._whatsapp_control.active_task_id = None
    ...
    def _whatsapp_control_task(self, command: RuntimeControlCommand, *, raw: str, source: str) -> Turn:
        """Apply the WhatsApp ownership lease without touching browser session state."""
        task = UserTask(raw=raw, text=raw, source=source, status="accepted")

        if command.kind is RuntimeControlKind.WHATSAPP_HANDOFF:
            active = self._whatsapp_control.active_task_id
            if active:
                return self._refused(
                    task,
                    "WHATSAPP_CONTROL_TRANSFER_BLOCKED: active_action_in_progress",
                )

            # Cancel only pending WhatsApp automation. The browser session itself
            # remains alive, logged in, and available to the human.
            cancelled = 0
            for runtime_task in self._runtime.list_active_tasks():
                if runtime_task.task_id == active:
                    continue
                resource = runtime_task.browser_resource_key
                goal = str(runtime_task.goal or "").casefold()
                if resource != "whatsapp" and "whatsapp" not in goal:
                    continue
                try:
                    self._runtime.cancel_task(runtime_task.task_id, reason="WhatsApp ownership transferred to human")
                    cancelled += 1
                except Exception:
                    # A concurrently terminal task is not a reason to fail the
                    # ownership transfer; RuntimeManager remains authoritative.
                    continue

            if self._whatsapp_intelligence is not None:
                self._whatsapp_intelligence.on_debug("WHATSAPP_INTELLIGENCE: OBSERVER_PAUSED owner=HUMAN")
                self._whatsapp_intelligence.stop_observing()
            self._whatsapp_control.release_to_human()
            if self.debug:
                self._debug_route("WHATSAPP_CONTROL", control="HUMAN")
            suffix = f" Pending WhatsApp tasks cancelled: {cancelled}." if cancelled else ""
            reply = "WHATSAPP_CONTROL: HUMAN_CONTROL. WhatsApp Web remains open and logged in; DEIMOS has released automation ownership." + suffix
            return self._refused(task, reply)

        if command.kind is RuntimeControlKind.WHATSAPP_TAKEOVER:
            if self._whatsapp_control.active_task_id:
                return self._refused(task, "WHATSAPP_CONTROL_TRANSFER_BLOCKED: active_action_in_progress")
            browser = self._whatsapp_control.browser
            if browser is None:
                # Establish the existing WhatsApp browser resource lazily. This is
                # still the same BrowserSkill path used by normal WhatsApp actions.
                browser = self._browser_for_task(
                    f"whatsapp-control-{uuid.uuid4().hex[:10]}",
                    resource_key="whatsapp",
                    allow_human_control_transfer=True,
                )
            try:
                # open_whatsapp() performs fresh semantic observations and verifies
                # that the authenticated WhatsApp Web UI is actually usable.
                result = browser.open_whatsapp()
                state = str(result.get("state") or "").upper()
                if state != "READY":
                    return self._refused(
                        task,
                        f"WHATSAPP_CONTROL_TAKEOVER_FAILED: unusable_whatsapp_state={state or 'UNKNOWN'}",
                    )
            except Exception as exc:
                self._whatsapp_control.owner = WhatsAppOwner.HUMAN
                return self._refused(
                    task,
                    f"WHATSAPP_CONTROL_TAKEOVER_FAILED: {getattr(exc, 'code', None) or type(exc).__name__}: {exc}",
                )

            self._whatsapp_control.acquire_for_deimos(browser)
            # Takeover is the explicit HUMAN -> DEIMOS boundary.  Always
            # resolve the persistent intelligence service here; it may not have
            # been instantiated in this Session even though intelligence was
            # enabled by an earlier Session.
            self._resume_whatsapp_intelligence_if_enabled()
            if self.debug:
                self._debug_route("WHATSAPP_CONTROL", control="DEIMOS")
            return self._refused(
                task,
                "WHATSAPP_CONTROL: DEIMOS_CONTROL. WhatsApp Web was freshly observed and is usable.",
            )

        return self._refused(task, "I could not resolve the WhatsApp control operation safely.")

    def _handle_runtime_control(self, command: RuntimeControlCommand, *, raw: str, source: str, background: bool) -> Turn:
        if command.kind is RuntimeControlKind.WHATSAPP_INTELLIGENCE_ENABLE:
            return self._enable_whatsapp_intelligence()
        if command.kind is RuntimeControlKind.WHATSAPP_INTELLIGENCE_DISABLE:
            return self._disable_whatsapp_intelligence()
        if command.kind is RuntimeControlKind.WHATSAPP_INTELLIGENCE_AUTHORIZE:
            return self._authorize_whatsapp_intelligence(command.scope)
        if command.kind is RuntimeControlKind.WHATSAPP_INTELLIGENCE_REVOKE:
            return self._revoke_whatsapp_intelligence(command.scope)
        if command.kind in {RuntimeControlKind.WHATSAPP_HANDOFF, RuntimeControlKind.WHATSAPP_TAKEOVER}:
            if self.debug:
                self._debug_input(InputEvent(text=raw, source=source), None, InputOwner.HUMAN, task_created=False)
            return self._whatsapp_control_task(command, raw=raw, source=source)
