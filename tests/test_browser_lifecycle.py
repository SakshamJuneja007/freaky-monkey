...
def test_independent_tasks_do_not_share_browser_resource_by_site():
    from agent_control.session import Session
    ...
    try:
        ...
        assert youtube_a is not youtube_b
        assert youtube_a is not whatsapp
        assert youtube_b is not whatsapp
    finally:
        session.close()



def test_task_browser_resource_reuses_only_same_task():
    from agent_control.session import Session
    ...
    session = Session(narrator=FakeNarrator(), _browser_backend=FakeBrowser("base"))
    try:
        first = session._browser_for_task("task-a", resource_key="youtube")
        same_task = session._browser_for_task("task-a", resource_key="whatsapp")
        other_task = session._browser_for_task("task-b", resource_key="youtube")
        assert same_task is first
        assert other_task is not first
    finally:
        session.close()
